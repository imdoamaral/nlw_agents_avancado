			...
			const chunks = await db
				...
				.where(
					and(
						...
						sql`1 - (${schema.audioChunks.embeddings} <=> ${embeddingAsString}::vector) > 0.7`, // grau de similaridade entre os textos
					),
				)
				.orderBy(
					sql`1 - (${schema.audioChunks.embeddings} <=> ${embeddingAsString}::vector)`,
				)
				.limit(3);
